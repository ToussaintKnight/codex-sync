...
usage() {
  echo "Usage: bootstrap-sync2.sh VAULT [DEVICE] [folder|git] [--daemon]" >&2
  exit 2
}
...
SOURCE="$VAULT/skills/codex/sync2"
DESTINATION="$CODEX_HOME/skills/sync2"

[ -f "$SOURCE/SKILL.md" ] || { echo "Sync2 skill was not found in vault: $SOURCE" >&2; exit 1; }
command -v node >/dev/null 2>&1 || { echo "Node.js 22 or newer is required." >&2; exit 1; }
MAJOR=$(node --version | sed 's/^v//' | cut -d. -f1)
[ "$MAJOR" -ge 22 ] || { echo "Sync2 requires Node.js 22 or newer." >&2; exit 1; }

mkdir -p "$DESTINATION"
cp -R "$SOURCE/." "$DESTINATION/"
CLI="$DESTINATION/scripts/sync2.mjs"
CONFIG=${SYNC2_CONFIG:-"$HOME/.sync2/config.json"}
...
if [ -f "$VAULT/.sync2/maintenance.json" ]; then
  echo "Sync2 maintenance mode detected; performing controlled pull and head refresh without enabling the scheduler."
