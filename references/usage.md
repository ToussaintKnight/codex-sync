...
| --- | --- | --- | --- | --- |
| Windows desktop | `win-main` | `D:\Sync2Vault` | `~/.codex/skills` | every 5 minutes |
| Windows laptop | `win-travel` | `E:\Sync2Vault` | `~/.codex/skills` | every 15 minutes |
| macOS | `mac-main` | `~/Sync2Vault` | `~/.codex/skills`, `~/.hermes/skills` | LaunchAgent installed |
...
| Request | Result |
| --- | --- |
| `$sync2 current` | Select the current Codex task, then synchronize it. |
| `$sync2` | Synchronize all selected tasks and configured skill roots now. |
| `$sync2 status` | Show local configuration, selected tasks, sources, conflicts, and scheduler state. |
| `$sync2 devices` | Show reports received from every bootstrapped device. |
| `$sync2 skills` | Discover common personal skill roots and synchronize them. |
| `$sync2 doctor` | Check Node, Codex storage, vault, SQLite index, Syncthing access, and forbidden files. |
...
- Windows: `%USERPROFILE%\.codex\skills\sync2\scripts\sync2.mjs`
- macOS: `~/.codex/skills/sync2/scripts/sync2.mjs`
- Local configuration: `~/.sync2/config.json`
- Local three-way state and recovery copies: `~/.sync2/state` and `~/.sync2/conflicts`
